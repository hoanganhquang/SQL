...
CREATE TABLE `products` (
  ...
  CONSTRAINT `products_ibfk_1` FOREIGN KEY (`productLine`) REFERENCES `productlines` (`productLine`) on update cascade
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
begin
	...
	update employees set lastName = 'Trump' where lastName = name;

end // 
delimiter ;

call change_name('Vanauf');
select * from employees where lastName = 'Trump';
...
begin
	...
	select status from orders where year(orderDate) = year;

end // 
delimiter ;

call check_status(2004);
...
begin
	...
	select customerName as "Khach hang", employees.firstName as "Ten nhan vien phu trach" 
    from customers 
...
delimiter ;
call get_customer_info('cancelled');


-- f. Lấy thông tin phòng ban nhân viên dựa vào mã employeeNumber
delimiter //
drop procedure if exists get_office_info //
create procedure get_office_info(
	in code int
)
begin

	select offices.* from employees
	inner join offices
...
begin
	...
	where products.productLine = name;

end // 
delimiter ;

call get_product_list('Classic Cars');
...
begin
	...
	select tb1.* from employees tb1
	inner join employees tb2
	on tb1.reportsTo = tb2.employeeNumber
	where tb2.employeeNumber = code;

end // 
delimiter ;

call get_employees(1002);
...
• Vintage Cars = > Xe cũ */
delimiter //
drop procedure if exists change_productline //
create procedure change_productline()
begin
    declare finished int default 0;
	declare _productline varchar(100) default "";
    declare curline 
		cursor for select productLine from productlines;
    ...
    changeLine: loop 
		fetch curLine into _productline;
        if finished = 1 then 
			leave changeLine;
		end if;
        
        if _productline = 'Classic Cars' then
			update productlines set productLine = 'Xe cổ' where productLine = _productline;
        elseif _productline = 'Motorcycles' then
			update productlines set productLine = 'Xe mô tô' where productLine = _productline;    
		elseif _productline = 'Planes' then
			update productlines set productLine = 'Máy bay' where productLine = _productline;
        elseif _productline = 'Ships' then
			update productlines set productLine = 'Tàu Thủy' where productLine = _productline;
        elseif _productline = 'Trains' then
			update productlines set productLine = 'Tàu Hỏa' where productLine = _productline;    
        elseif _productline = 'Trucks and Buses' then
			update productlines set productLine = 'Xe Tải và Buýt' where productLine = _productline;    
		elseif _productline = 'Vintage Cars' then
			update productlines set productLine = 'Xe cũ' where productLine = _productline;
		end if;
...
-- call change_productline();
select * from productlines;


-- j. Như yêu cầu trong câu h nhưng sử dụng CASE
delimiter //
drop procedure if exists change_productline1 //
create procedure change_productline1()
begin
    declare finished int default 0;
	declare _productline varchar(100) default "";
    declare curline 
    ...
    changeLine: loop 
        ...
        case _productline 
			when 'Classic Cars' then
				update productlines set productLine = 'Xe cổ' where productLine = _productline;
			when 'Motorcycles' then
				update productlines set productLine = 'Xe mô tô' where productLine = _productline;    
			when 'Planes' then
				update productlines set productLine = 'Máy bay' where productLine = _productline;
			when 'Ships' then
			...
			when'Trains' then
				update productlines set productLine = 'Tàu Hỏa' where productLine = _productline;    
			when 'Trucks and Buses' then
				update productlines set productLine = 'Xe Tải và Buýt' where productLine = _productline;    
			when 'Vintage Cars' then
				update productlines set productLine = 'Xe cũ' where productLine = _productline;
		end case;
...
call change_productline1();
select * from productlines;


-- k. Lấy ra dòng sản phẩm dựa theo tên của một sản phẩm
delimiter //
drop procedure if exists get_productLine //
create procedure get_productLine(
	in name varchar(50)
)
begin

	select productlines.* from products 
	inner join productlines
	on products.productLine = productlines.productLine
...
call get_productLine('1958 Setra Bus');


-- l. Sử dụng vòng lặp liệt kê các sản phẩm thuộc dòng Xe cũ có giá mua thấp hơn 50
...
delimiter //
drop procedure if exists _products_list //
create procedure _products_list()
begin
	declare finished int default 0;
	declare _productName varchar(100) default "";
    declare _productLine varchar(100) default "";
    declare _buyPrice int default 0;
    declare curline 
		cursor for select productName, productLine, buyPrice from products where productLine = 'Xe cũ' and buyPrice < 50;
    declare continue handler  
		for not found set finished = 1;
        
	open curline;
    getName: loop 
		fetch curLine into _productName, _productLine, _buyPrice;
        if finished = 1 then 
			leave getName;
		end if;
        
        insert into Cars(name, productLine, buyPrice) values (_productName, _productLine, _buyPrice);
...
-- Viết Trigger thực hiện các yêu cầu sau:
...
delimiter //
drop trigger if exists email_err //
create trigger email_err after insert on employees for each row
begin

	if new.email is null then 
		insert into message(employeeId, message) values (new.employeeNumber, 'Quên nhập email');
    end if;
...
insert  into `employees`(`employeeNumber`,`lastName`,`firstName`,`extension`,`email`,`officeCode`,`reportsTo`,`jobTitle`) values 
(1705,'tab','pro','x5800', null,'1',NULL,'President');
select * from message;


-- b. Thông báo lỗi không cho phép sửa tên của các dòng sản phẩm
delimiter //
drop trigger if exists productline_name_err //
create trigger productline_name_err before update on productlines for each row
begin

	if new.productLine is not null then 
		signal sqlstate '45000' set message_text = 'Không cho phép sửa tên';
    end if;

end //
delimiter ;
select * from productlines;
update productlines set productLine = "hay" where productLine = 'Máy bay';
