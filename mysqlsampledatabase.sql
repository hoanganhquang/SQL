...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`1812830_database_lab02` /*!40100 DEFAULT CHARACTER SET utf8mb4 */;

USE `1812830_database_Lab02`;
...
where orderdetails.orderNumber = orders.orderNumber limit 1) as ProductCode
from orders where month(orderDate) = 5 and year(orderDate) = 2003;

-- b. Đưa ra các thông tin về các đơn hàng trong các tháng của năm 2005
select tb.orderDate, (select productName from products where products.productCode = tb.productCode) 
as "Product Name" from
(select orders.orderDate, orderdetails.* from orders
inner join orderdetails on orders.orderNumber = orderdetails.orderNumber
where year(orderDate) = 2005) tb;

-- c. Đưa ra thông tin về các đơn hàng và tổng giá trị đơn hàng
select orderNumber,
(select sum(quantityOrdered * priceEach) from orderdetails tb2
where tb1.orderNumber = tb2.orderNumber) 
as "Tong gia tri don hang" from orderdetails tb1 group by orderNumber;

-- d. Đưa ra thông tin về đơn hàng có tổng giá trị đơn hàng nhỏ nhất
select min(total) "Don hang co tong gia tri nho nhat" from
( select orderNumber, (select sum(quantityOrdered * priceEach) 
from orderdetails tb2 
where tb1.orderNumber = tb2.orderNumber) 
as total from orderdetails tb1 group by orderNumber) as rerult;

-- e. Với mỗi khách hàng, đưa ra tổng số hàng, và tổng số tiền họ đã thanh toán
select sum(quantityOrdered) as "Tong so hang", 
(sum(quantityOrdered * priceEach)) as "Tong tien da thanh toan", 
(select customerNumber from customers 
where customers.customerNumber = orders.customerNumber) as "CustomerNumber"
from orders inner join orderdetails 
on orders.orderNumber = orderdetails.orderNumber 
group by orders.orderNumber;

-- f. Liệt kê họ tên khách hàng cùng số lượng các đơn bị hủy của họ trong năm 2003.
select count(*) as "So don hang bi huy", 
(select concat_ws(" ", contactFirstName, contactLastName) 
from customers where customers.customerNumber = o.customerNumber) as "Khach hang"
from orders o where status like "cancelled" and year(orderDate) = 2003 group by customerNumber;

-- g. Đưa ra các mã đơn đặt hàng có giá trị lớn nhất không thuộc dòng sản phẩm “Classic Cars”.
select * from (
select *, (select productLine from products where products.productCode = o.productCode) as productLine
from orderdetails o 
where orderNumber = (select max(orderNumber) from orderdetails)
) as result where result.productLine not like "Classic Cars";

-- h. Đưa ra mã nhóm hàng, tên nhóm hàng và tổng số lượng hàng hoá còn trong kho của nhóm hàng đó.
select productLine, 
(select sum(quantityInStock) from products tb1 
where tb1.productLine = tb2.productLine) as "Tong so luong hang hoa con trong kho" 
from products tb2 group by productLine;

-- i. Đưa ra thông tin về các nhân viên và tên văn phòng nơi họ làm việc.
select *, (select concat_ws(" -- ", city, phone) from offices
where offices.officeCode = employees.officeCode
) as "Thong tin van phong" from employees;

-- j. Đưa ra thông tin về nhân viên mà văn phòng nơi họ làm việc có tổng các
-- chữ số thành phần của số điện thoại là nhỏ nhất.
-- Tạo table chứa giá trị tổng của chữ số điện thoại và procedure 
-- tính tổng các chữ số điện thoại 

drop table if exists sumPhoneNumber;
create table sumPhoneNumber (
...
create procedure sumDigit()
begin
	declare result bigint;
    declare n int;
    declare finished int;
    declare curPhone cursor for select replace(substring(phone, 2), " ", "") from offices;
    declare curOfficeCode cursor for select 
    ...
    getPhone: loop
		...
		while (result > 0) do
			set n = n + (result mod 10);
			set result = result div 10;
		end while;
		insert into sumPhoneNumber(sum) values(n);
        set n = 0;
    end loop getPhone;
	close curPhone;
...
DELIMITER ;
call sumDigit();
select * from sumPhoneNumber;

select *, (select sum from sumPhoneNumber where sumPhoneNumber.id = employees.officeCode) as total
from employees where (select sum from sumPhoneNumber where sumPhoneNumber.id = employees.officeCode) = (select min(sum) from sumPhoneNumber); 


-- k. Đưa ra thông tin về tên khách hàng và tên các sản phẩm họ đã mua không có chứa chữ “Ford” nhưng có chứa chữ “A”.
select tb.productName, (select customerName from customers where customers.customerNumber = tb.customerNumber) as "CustomerName" from
(select products.productName, orders.customerNumber from orderdetails 
inner join orders on orders.orderNumber = orderdetails.orderNumber
inner join products on products.productCode = orderdetails.productCode
where products.productName not like "%Ford%" and productName like "%A%") as tb;
...
where month(orderDate) = 3 and year(orderDate) = 2005 and orders.orderNumber = result.orderNumber) date 
from 
(select sum(quantityOrdered * priceEach) as total, orderNumber 
from orderdetails group by orderNumber) as result
where total > 10000 and total <= 40000 and 
(select orderDate from orders 
where month(orderDate) = 3 and year(orderDate) = 2005 and orders.orderNumber = result.orderNumber) is not null
;

-- n. Đưa ra thông tin về các dòng sản phẩm và các sản phẩm của từng dòng sản
-- phẩm đó. Sắp xếp giảm dần theo thứ tự abc.
select productName, (select productLine from productlines where productlines.productLine = products.productLine) as productLine
...
select productName, (select productLine from productlines where productlines.productLine = products.productLine) as productLine
from products where productLine not like "Motorcycles" and char_length(productName) <= 25;

-- ------------------------ Truy van con khong tuong quan -----------------------------------
-- a. Lấy ra các sản phẩm có đơn đặt hàng trong tháng 5/2003.
select orderdetails.orderNumber, tb1.orderDate, orderdetails.productCode
from (select orderDate, orderNumber from orders where month(orderDate) = 5 and year(orderDate) = 2003) 
as tb1 inner join orderdetails on tb1.orderNumber = orderdetails.orderNumber;

-- b. Đưa ra các thông tin về các đơn hàng trong các tháng của năm 2005
select tb.orderDate, products.productName from
(select orders.orderDate, orderdetails.* from orders
inner join orderdetails on orders.orderNumber = orderdetails.orderNumber
where year(orderDate) = 2005) tb inner join products on products.productCode = tb.productCode;

-- c. Đưa ra thông tin về các đơn hàng và tổng giá trị đơn hàng
select tb1.orderNumber, tb2.total from orderdetails tb1
inner join (select sum(quantityOrdered * priceEach) as total, orderNumber from orderdetails group by orderNumber) tb2
on tb1.orderNumber = tb2.orderNumber group by tb1.orderNumber;

-- d. Đưa ra thông tin về đơn hàng có tổng giá trị đơn hàng nhỏ nhất
select min(total) "Don hang co tong gia tri nho nhat" from
( select tb1.orderNumber, tb2.total from orderdetails tb1
inner join (select sum(quantityOrdered * priceEach) as total, orderNumber from orderdetails group by orderNumber) tb2
on tb1.orderNumber = tb2.orderNumber group by tb1.orderNumber) as rerult;

-- e. Với mỗi khách hàng, đưa ra tổng số hàng, và tổng số tiền họ đã thanh toán 
select tb1.totalProduct, tb1.totalPrice, customerNumber from 
(select orderNumber, sum(quantityOrdered) as totalProduct, sum(quantityOrdered * priceEach) as totalPrice 
from orderdetails group by orderNumber) tb1
inner join orders on tb1.orderNumber = orders.orderNumber;


-- f. Liệt kê họ tên khách hàng cùng số lượng các đơn bị hủy của họ trong năm 2003.
select concat_ws(" ", customers.contactFirstName, customers.contactLastName) as CustomerName, tb1.totalCancel from
(select count(*) as totalCancel, customerNumber from orders o where status like "cancelled" and year(orderDate) = 2003 group by customerNumber) tb1
inner join customers on tb1.customerNumber = customers.customerNumber
;
...
-- h. Đưa ra mã nhóm hàng, tên nhóm hàng và tổng số lượng hàng hoá còn trong kho của nhóm hàng đó.
select productLine, total from 
(select productLine, sum(quantityInStock) as total from products group by productLine) as tb;


-- i. Đưa ra thông tin về các nhân viên và tên văn phòng nơi họ làm việc.
select employeeName, officeInfo from
(select concat_ws(" ", employees.firstName, employees.lastName) as employeeName, concat_ws(" -- ", offices.city, offices.phone) as officeInfo from employees 
inner join offices on employees.officeCode = offices.officeCode) as result;

-- j. Đưa ra thông tin về nhân viên mà văn phòng nơi họ làm việc có tổng các
-- chữ số thành phần của số điện thoại là nhỏ nhất.
select employees.*, sumPhoneNumber.sum as SumPhoneNumber 
from employees 
inner join sumPhoneNumber 
on employees.officeCode = sumPhoneNumber.id
where sumPhoneNumber.sum = (select min(sum) from SumPhoneNumber)
;

-- k. Đưa ra thông tin về tên khách hàng và tên các sản phẩm họ đã mua không có chứa chữ “Ford” nhưng có chứa chữ “A”.
select customerName, tb2.productName from 
(select orders.*, customers.customerName from orders inner join customers on orders.customerNumber = customers.customerNumber) as tb1
inner join 
(select orderDetails.*, products.productName from products
inner join orderDetails on orderdetails.productCode = products.productCode
where productName like "%Ford%" and productName not like "%A%") as tb2 on tb1.orderNumber = tb2.orderNumber;

-- l. Đưa ra thông tin về các mặt hàng có ít nhất một người đặt mua trong năm
-- 2005 nhưng không mua trong năm 2004.
select tb2005.*, tb2004.orderDate from
(select orders.orderNumber, orders.orderDate, orderdetails.productCode from orders 
inner join orderdetails on orders.orderNumber = orderdetails.orderNumber
where year(orderDate) = 2005) as tb2005 
left join 
(select orders.orderNumber, orders.orderDate, orderdetails.productCode from orders 
inner join orderdetails on orders.orderNumber = orderdetails.orderNumber
where year(orderDate) = 2004) as tb2004 on tb2005.productCode = tb2004.productCode
where tb2004.orderDate is null;
;

-- m. Đưa ra thông tin về các đơn hàng trong tháng 3/2005 và tổng giá trị của mỗi
-- đơn hàng lớn hơn 10 000 nhưng không vượt quá 40 000.
select orderDate, total as "Tong gia tri" from (
select orderdetails.orderNumber, sum(orderdetails.quantityOrdered * orderdetails.priceEach) as
total, orders.orderDate, orders.requiredDate, orders.status from orderdetails
inner join orders on orders.orderNumber = orderdetails.orderNumber
where month(orders.orderDate) = 3 and year(orders.orderDate) = 2005
group by orderdetails.orderNumber) as result where total > 10000 and total <= 40000;
;

-- n. Đưa ra thông tin về các dòng sản phẩm và các sản phẩm của từng dòng sản
-- phẩm đó. Sắp xếp giảm dần theo thứ tự abc.
select productName, tb.productLine from (select productLine from productlines) as tb
inner join products on products.productLine = tb.productLine order by products.productLine;

-- o. Đưa ra thông tin về các sản phẩm không thuộc dòng sản phẩm Motorcycles
-- mà số ký tự trong tên sản phẩm không vượt quá 25.
select productName, tb.productLine from (select productLine from productlines) as tb
inner join products on products.productLine = tb.productLine 
where tb.productLine not like "Motorcycles" and char_length(productName) <= 25
